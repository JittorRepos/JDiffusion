...
class TuneAVideoPipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image_pil, device, num_images_per_prompt, do_classifier_free_guidance, img_proj=None):
        ...
        # # image encoding
        clip_image_mean = jittor.array(self.feature_extractor.image_mean)[:,None,None].to(device, dtype=torch.float32)
        clip_image_std = jittor.array(self.feature_extractor.image_std)[:,None,None].to(device, dtype=torch.float32)
        temp_image = PIL.Image.fromarray((image_pil.squeeze().numpy() * 255).astype(np.uint8))
        imgs_in_proc = TF.resize(temp_image, (self.feature_extractor.crop_size['height'], self.feature_extractor.crop_size['width']))
        imgs_in_proc = jittor.array(TF.to_tensor(imgs_in_proc) / 255.0).unsqueeze(0)
        ...
        image_pil = image_pil.permute(0, 3, 1, 2)
